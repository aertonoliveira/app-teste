  ...
  useEffect(() => {
    async function loadExtracts() {
      if (isFocused) {
        ...
        setAmount(
          response.data.toLocaleString("pt-br", {
    ...
    async function loadExtractSum() {
      const response = await api.get(`getpaymentsallsum?id=${userId}`);

      setSum(response.data.value);
    }
    ...
    <ContainerPrice>
      <Info>
        <BalanceContainer>
          ...
          <Balance>
            R$ <BalanceSubTitle> {amount}</BalanceSubTitle>
          </Balance>
        </BalanceContainer>
      ...
      <BalanceBold>
        Lançamentos Futuros: R$ <Balance>{sum}</Balance>
      </BalanceBold>
        ...
        {!budgetConfirmed ? (
          <ModalBudgetSubContainer>
            <BudgetContainer>
              ...
              />
              <BudgetTitle>Saldo disponível R$ {amount}</BudgetTitle>
            </BudgetContainer>
